...
int GetValue(std::shared_ptr<Object> obj, std::unordered_map<std::string, std::string> *scope) {
  int ans;
...
bool IsList(const std::string& str) {
  std::stringstream s{str};
  Tokenizer tokenizer{&s};
  auto root = Read(&tokenizer);
  if (auto temp = As<Cell>(root)) {
...
std::vector<Func> GetFunc(std::shared_ptr<Cell> root) {
    ...
    if (auto var = As<Symbol>((*this)[1])) {
      ...
      if (auto val = As<Cell>((*this)[2])) {
        ...
        if (func_->find(As<Symbol>(list[0])->GetName()) != func_->end()) {
          ...
          for (int ind = 0; ind < funcs.size(); ++ind) {
            funcs[ind].dict_ = dict;
          }
...
std::string SchemeList::ListOperEval(std::string oper) {
  if (oper == "list") {
    ...
    while ((*this)[ind]) {
      if (auto temp = As<Number>((*this)[ind])) {
        if (!ans.empty()) {
          ans = ans + " " + std::to_string(temp->GetValue());
        } else {
          ans = std::to_string(temp->GetValue());
        }
      }
